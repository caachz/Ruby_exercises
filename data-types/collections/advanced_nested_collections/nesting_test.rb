...
class NestedTest < MiniTest::Test
  ...
  def test_list_of_dishes_names_for_olive_garden
    #=======================
    dishes_names = []
    dishes_names = stores[:olive_garden][:dishes].each {|x| return  x[:name]}
    #dishes_names = dishes_names.push(stores[:olive_garden][:dishes][0][:name])
    #dishes_names = dishes_names.push(stores[:olive_garden][:dishes][1][:name])
    #=======================
  ...
  def test_list_of_employees_across_all_restaurants
    #=======================
    employee_names = stores.each {|x, y| return y[:employees]}
    #=======================
  ...
  def test_list_of_all_ingredients_across_all_restaurants
    #=======================
    ingredients = stores.each {|x, y| return  y[:dishes].each {|z| return z[:ingredients]}}
    #=======================
    assert_equal ["Rice",
                  "Cheese",
                  "Butter",
  ...
  def test_full_menu_price_for_olive_garden
    #=======================
    full_menu_price = stores[:olive_garden][:dishes].each {|x| return x[:price]}.sum
    #=======================
  ...
  def test_full_menu_for_olive_garden
    ...
    stores[:olive_garden][:dishes].each do |dish|
      hash_of_dishes[dish[:name]] = dish
    end
    olive_garden_menu = hash_of_dishes
    #=======================
  ...
  def test_menu_accross_all_restaurants
    ...
    stores.each do |store|
      store[1][:dishes].each do |dish|
        hash_of_all_dishes[dish[:name]] = dish
      end
    end
      full_menu = hash_of_all_dishes
    #=======================
    expected = ({"Risotto"=>
